...
boolean Plugin_200(byte function, struct EventStruct *event, String& string)
{
  ...
  switch (function)
  {
    ...
    case PLUGIN_WEBFORM_LOAD:
    {
      ...
      byte addr                                      = PCONFIG(0);
      const uint8_t optionValues[ADS1115_I2C_OPTION] = { 0x48, 0x49, 0x4A, 0x4B };
    ...
    case PLUGIN_ONCE_A_SECOND:
    {
      ...
      uint16_t gain   = PCONFIG(1);
      int16_t  raw    = ADC_MAX;

      //we read using the highest gain but if we are out of range we decrement the gain
      //this allows to choose a higher gain than needed and the plugin calibrates itself to the correct one
      while (raw >= ADC_MAX || raw <= ADC_MIN)
      {
        uint16_t mux = PCONFIG(2);
        p200_prepareRead(address, gain,mux);
        delay(8);
      ...
      uint16_t filter_power = (uint16_t)PCONFIG_LONG(1);
      float avg;
      int16_t lastGain= PCONFIG(1);
      //we check to see if the gain has changed during the current read
      if (lastGain == gain) //no change
      {
      ...
      {
        //the gain have changed so we need to save the current gain and start over
        PCONFIG(1) = gain;
        avg = (float)raw;
      ...
      uint32_t Vref = 6600 << gain;        // calculate the 3v3 /2  adc value
      ...
      */
      UserVar[event->BaseVarIndex+2] = avg / (float)Vref; // compensate for the 2^gain gain
    ...
    case PLUGIN_READ:
    {
      ...
      #define T0 298.15
      double temperature = (beta * T0)/ (beta + T0*ln)- 273.15; //in celsius
