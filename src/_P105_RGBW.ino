...
#ifdef ESP32
  #define analogWrite analogWriteESP32
#endif
...
/*
A good graph for the RGB values relative: http://www.vendian.org/mncharity/dir3/blackbody/UnstableURLs/bbr_color.html
I fitted the RGB values:
 X= Kelvin /100

	RED =>
	   from x=100->660 red=1
	   from x >660 red = -0.14*ln(x) + 1.0983 (R² = 0.9763)
	   		       red =  1.4662*x^-0.265 (R² = 0.9791)

	GREEN => 
	    from x=100->660 green =-0.0001*X^2+0.0244*X+0.0067 (R² = 0.9998)
					    green =            0.0161*x + 0.1 (R² = 0.9842)
		
		from X>660 		green = -0.099*ln(x) + 1.052 (R² = 0.9821)
						green = -5E-08*x^3 + 3E-05*x^2 - 0.0055*x + 0.8892 (R² = 0.9847)

	BLUE =>
	    from x=100->200  blue =0
		from x=200->660  blue =3E-05*x^2 + 0.0227*x - 0.02 (R² = 0.9996)
						 blue =             0.024*x - 0.0286 (R² = 0.9993)
		from x>660       blue =1
...
void colorTemperatureToRGB(int16_t kelvin){
    ...
    if( temp <= 66 ){ 
        ...
        if( temp <= 19){
			...
			blue =0.024*temp - 0.0286; // (R² = 0.9993)
        }
        ...
        red =  1.4662*pow(temp,-0.265); // (R² = 0.9791)
        //green = temp - 60;
...
boolean Plugin_105(byte function, struct EventStruct *event, String& string)
{
	...
	//	if (argIndex) tmpString = tmpString.substring(0, argIndex);
        ...
        Par[1] = parseString(string,2).toInt();
        Par[2] = parseString(string,3).toInt();
        Par[3] = parseString(string,4).toInt();
        Par[4] = parseString(string,5).toInt();
        Par[5] = parseString(string,6).toInt();
        Par[6] = parseString(string,7).toInt();
        Par[7] = parseString(string,8).toInt();
